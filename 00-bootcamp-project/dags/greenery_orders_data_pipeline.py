...
def _extract_data(ds):
    extract_data_api("orders", ds, True)

def _load_data_to_gcs(ds):
    upload_data_to_gcs("orders", ds)

def _load_data_from_gcs_to_bigquery(ds):
    load_partition_tables("orders", ds)


default_args = {
    "owner": "airflow",
		"start_date": timezone.datetime(2021, 1, 1),  # Set an appropriate start date here
}
with DAG(
